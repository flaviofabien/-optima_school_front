...
export default function EmploiDuTemps() {
    ...
    useEffect(() => {
        if (dataCourse && currentViewDate) {
            // 
            const formattedEvents = dataCourse.filter((i) => {
                if (user.role === "admin") return i.Salle.id == dataInclude.salle.id ;
                else if (user.role === "eleve") return i;
            } 
             ).map((course): EventInput | null => {
