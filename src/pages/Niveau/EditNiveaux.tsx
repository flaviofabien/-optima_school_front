...
export default function EditNiveau() {
    ...
    const {data,isLoading : userOneIsLoading ,isError : userOneIsError} = useQuery<any>({
    ...
    const mutation = useMutation(
        {
        mutationFn: (newUser : any) => UpdateNiveau(token,newUser,id!),
        onSuccess: () => {
            setLoad(true)
            setErrorServer("");
    ...
    <div className="">
        <div className="mt-4 w-full flex justify-center items-center">
            <div className="w-full max-w-[1200px] rounded-3xl bg-white mt-8 flex justify-center items-center" >
                <form className=" h-full w-full lg:w-[500px] relative rounded-s-3xl" onSubmit={handleSubmit(onSubmit)} >
                ...
                <img src={ImgFontLogo} className="lg:block hidden w-full h-[662px]  object-cover  rounded-e-3xl" alt="" />
            </div>
        </div>
    </div>
  )
