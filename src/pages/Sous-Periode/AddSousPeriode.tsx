...
import { CreateSousPeriode } from "../../api/Sous-periode"
import { SousPeriodeSchema } from "../../Zod-Validation/SousPeriode"

export default function AddSousPeriode() {
    ...
    const mutation = useMutation(
        {
        mutationFn: (newUser : any) => CreateSousPeriode(token,newUser),
        onSuccess: () => {
            setErrorServer("");
            dispatch(setAlert({status : true,message : `Sous Periodes a ete ajouter avec succes`}))
        ...
        <div className="mt-4 w-full flex justify-center px-8 items-center">
            <div className="w-full max-w-[1200px] rounded-3xl bg-white mt-8 flex justify-center items-center" >
                <form className=" h-full w-[500px] relative rounded-s-3xl" onSubmit={handleSubmit(onSubmit)} >
                    ...
                    <TitleForm title="Ajoute Sous periode" />
