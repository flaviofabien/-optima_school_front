...
export default function Pagination({paramsPatient,setParamsPatient,totalPage} : Props) {
  return (
    <div className="w-full justify-center flex mb-4 mt-8">
              ...
              className="text-lg bg-black px-2 py-1 text-white rounded-lg">
                  Prec</button>  }
                  {
                        Array.from({ length:totalPage  }, (_, i) => i + 1).map((i : number) => (
                                <button key={i} onClick={() => setParamsPatient((prec) => ({ ...prec , page : i})  ) } className={` p-4 rounded-lg cursor-pointer shadow-md ${i === paramsPatient.page ? "bg-[var(--color-primary)] text-white"  : "bg-gray-200  text-black"}  `}>  {i} </button>
                        ))
                    }
