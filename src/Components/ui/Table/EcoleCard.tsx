...
export default function EcoleCardContainer({   FnQueryGet , query ,title , functionMutation ,dataFilterSelect }: Props) {
  ...
  return ( 
  <div className=' w-full lg:pl-64 pr-8  '>
                  ...
                  <div className="flex flex-col gap-2  w-full max-w-[290px] bg-white rounded-2xl shadow-md hover:shadow-xl transition-shadow duration-300 transform hover:-translate-y-1">
                      ...
                      <div className=" w-full p-4">
                        ...
                        <div className="space-y-2 text-sm text-gray-700">
                          ...
                          <div className="flex justify-between flex-wrap items-center border-t pt-2">
                               {i?.niveaux?.map((i : any) => <span> {i.nom} </span> )}
                          </div>
                            <p className="">
                                <BiEdit onClick={() => navigate(`/admin/${query}/edit/${i.id}`)} className="inline-block mr-4 text-2xl text-blue-500" />
                                <MdDelete onClick={() => setShow({ id: i.id, show: true })} className="inline-block text-2xl text-red-500" />
                            </p>
