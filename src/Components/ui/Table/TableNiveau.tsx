...
export default function TableNiveau({ columns , FnQueryGet , query ,title , functionMutation  }: Props) {
            ...
            <table className="overflow-auto w-full ">
              <thead className="text-lg text-white">
                    <tr className="">
                      ...
                      <td className="lg:px-6 px-4 py-4  font-medium "> Action </td>
                    </tr>
              </thead>
            <tbody>
              {data?.data.map((item: any,i : any) => (
                <tr key={item.id}  className={`   w-96 border-b-2 border-[var(--color-primary-transparent)] ${i % 2 === 0 ? "bg-[var(--white)] " : "bg-slate-200"}  `}>
                  {columns.map((column, index) => (
